...
class NotionAPI:
            ...
            rss_feeds = [
                {
                    ...
                    "AiSummaryEnabled": item["properties"]["AiSummaryEnabled"][
                        "checkbox"
                    ],
                    "tags": [
                        tag["name"]
    ...
    def is_page_exist(self, page_link, database_id):
        ...
        query = {"filter": {"property": "link", "url": {"equals": page_link}}}
        response = self.notion.databases.query(database_id=database_id, **query)
    ...
    def create_article_page(self, rss, entry, database_id, md):
        ...
        try:
            ...
            if page_id:
                logging.info(
                    f"文章 '{entry['title']}' 的首批100个块已成功保存到Notion。"
                )
                # 如果还有更多块需要添加，继续添加剩余块
                remaining_blocks = blocks[100:]
                while remaining_blocks:
                    self.notion.blocks.children.append(
                        block_id=page_id, children=remaining_blocks[:100]
                    )
            ...
            logging.error(
                f"创建文章页面时出错，rss_name: {rss['title']}, article_title: {entry['title']}, exception:{e}"
            )
        return None
    ...
    def update_rss_status(self, rss_id, status):
        """更新RSS源状态"""
        try:
            update_data = {"properties": {"status": {"select": {"name": status}}}}
            self.notion.pages.update(page_id=rss_id, **update_data)
            logging.info(f"RSS源 {rss_id} 状态更新为 {status}.")
        except Exception as e:
            logging.error(
                f"更新RSS源状态出错，rss_id: {rss_id}, status: {status}, exception:{e}"
            )
        ...
        try:
            update_data = {
                "properties": {
                    ...
                    "updated": {"date": {"start": updated}},
                    "name": {"title": [{"text": {"content": title}}]},
                }
            }
            self.notion.pages.update(page_id=rss["id"], **update_data)
            logging.info(
                f"RSS源 {rss['id']} 状态更新为 {status}. 更新时间: {updated}, 名称: {title}"
            )
        except Exception as e:
